...
### getting the sector along wih the symbols
sector_data <- tq_index("SP500") %>% select(symbol, sector)
...
keep_wf <- keep_close_c %>% 
  dplyr::select(symbol, close, day_number, sector) %>% 
  tidyr::spread(day_number, close)

### save the long and wide format data sets
### uncomment the code below to save the data as CSVs
keep_close_c %>%
  readr::write_csv("sp500_stock_april_close_with_sector_long_format.csv", col_names = TRUE)

keep_wf %>%
  readr::write_csv("sp500_stock_april_close_with_sector_wide_format.csv", col_names = TRUE)
